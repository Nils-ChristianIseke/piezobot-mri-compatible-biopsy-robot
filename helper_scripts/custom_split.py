...
def list_files_without_extension(
    directory_path, file_extension: str = ".nii.gz"
) -> None:
    files = os.listdir(directory_path)
    file_extension_length = len(file_extension)
    file_names_without_extension = [
        file[:-file_extension_length] for file in files if file.endswith(file_extension)
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--number_of_folds", type=int, default=5, help="Number of Folds"
    )
    parser.add_argument(
        "--file_extension",
        type=str,
    ...
    parser.add_argument(
        "--path_to_raw_files", type=str, help="Path where the files are listed"
    )
    parser.add_argument(
        "--path_to_splits_file",
        type=str,
    ...
    file_names = list_files_without_extension(path_to_raw_files, file_extension)
